...
export default function CityServicesPage() {
  ...
  const fetchServices = useCallback(async () => {
    ...
    setError(null);
    try {
      const res = await apiFetch("/api/city-services");
  ...
  async function handleBanService(service: CityService) {
    ...
    try {
      const res = await apiFetch(`/api/city-services/${service.id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  async function handleDeleteCity(id: string) {
    ...
    try {
      const res = await apiFetch(`/api/cities/${id}`, { method: "DELETE" });
            ...
            <div className="flex justify-end gap-2">
              ...
              <button
                onClick={() => {
                  if (deleteConfirm) {
                    setDeleting(true);
                    apiFetch(`/api/city-services/${deleteConfirm}`, { method: "DELETE" })
